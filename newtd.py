...
class CompleteScan(threading.Thread):
    ...
    def run(self):
		cmd = "sudo ip netns exec qdhcp-" + self.namespace + " nmap " + self.hostaddress + " -A -O -oX /opt/stack/completescanresult/" + str("qdhcp-" + self.namespace + "-" + self.opfilename) + ".xml"
		print(cmd)
		os.system(cmd)
		self.returnvalue=str("qdhcp-" + self.namespace + "-" + self.opfilename) + ".xml" + " HAS BEEN CREATED"
			...
			for key2 in netip:
					if key == key2:
							...
							for element in list1:	
									temp = element
									temp = temp.replace(".","-")
									threadList.append(CompleteScan(network_id[key],element,temp))
									i=i+1	
